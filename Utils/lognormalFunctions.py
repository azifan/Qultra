...
def data_fit(TIC,normalizer, autoT0):
    ...
    if not autoT0:
        ...
        wholeCurve = lognormal(TIC[0], normalizedLogParams[0], normalizedLogParams[1], normalizedLogParams[2])
    else:
        normalizedLogParams, normalizedLogParamCov = curve_fit(lognormal_t0, TIC[0], TIC[1], p0=(1.0,0.0,1.0,0.0),bounds=([0.,0., 0., 0.], [np.inf, np.inf, np.inf, np.inf]),method='trf')#p0=(1.0,3.0,0.5,0.1) ,**kwargs
        t0 = normalizedLogParams[3]
        wholeCurve = lognormal_t0(TIC[0], normalizedLogParams[0], normalizedLogParams[1], normalizedLogParams[2], t0)
