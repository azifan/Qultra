...
def roiWindowsGenerator(
    xspline,
    yspline,  # The spline parameters
    axialNum,
    lateralNum,  # Number of pixels along the axial and lateral sides of the image
    axialRSize,
    lateralRSize,  # Window size in mm - user inputted via GUI
    axialRes,
    lateralRes,  # Axial and lateral image resolution in mm/pixel - width/lines, height/samples
    axialOverlap,
    ...
    for axialPos in np.arange(axialStart, axialEnd, axialIncrement):
        for lateralPos in np.arange(lateralStart, lateralEnd, lateralIncrement):
            ...
            maskVals = mask[
                axialInd : (axialInd + axialRoiSizePixels),
                lateralInd : (lateralInd + lateralRoiSizePixels),
            ]
            ...
            if percentageOnes > (thresholdPercentage / 100):
                ...
                roiPositions.right.append(
                    int(lateral[lateralInd + lateralRoiSizePixels - 1])
                )
                roiPositions.top.append(int(axial[axialInd]))
                roiPositions.bottom.append(
                ...
                if len(xmap) != 0:
                    ...
                    roiPositionsPreSC.left.append(
                        int(
                            np.floor(
                                xmap[int(np.floor(axialInd)), int(np.floor(lateralInd))]
                            )
                        )
                    )
                    roiPositionsPreSC.right.append(
                        int(
                            np.ceil(
                                xmap[
                                    int(np.floor(axialInd)),
                                    int(
                                        np.floor(lateralInd + lateralRoiSizePixels - 1)
                                    ),
                                ]
                            )
                        )
                    ...
                    roiPositionsPreSC.top.append(
                        int(
                            np.floor(
                                ymap[int(np.floor(axialInd)), int(np.floor(lateralInd))]
                            )
                        )
                    )
                    roiPositionsPreSC.bottom.append(
                        int(
                            np.floor(
                                ymap[
                                    int(np.ceil(axialInd + axialRoiSizePixels - 1)),
                                    int(np.floor(lateralInd)),
                                ]
                            )
                        )
                    )
