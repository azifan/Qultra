...
def find_ref_frames_from_nifti(nifti_path, ROI_margin_suffix, search_margin):
    ...
    bboxes = []
    masks = []
    for _, ref_frame in enumerate(ref_frames):
        ...
        masks.append(
            mask[
                y0 - ROI_margin : y0 - ROI_margin + h + (2 * ROI_margin),
...
def getIndTIC(intensities, mode, availableFrames, pixelScale, times):
    ...
    tic = np.concatenate(
        [availableFrames[:, np.newaxis], availableIntensities[:, np.newaxis]], axis=1
    )
...
def getAllTICs(ceMcRoi, pixelScale, times):
    ...
    for x in range(ceMcRoi.shape[1]):
        for y in range(ceMcRoi.shape[2]):
            ticArray[x, y] = getIndTIC(
                ceMcRoi[:, x, y], "mc", availableFrames, pixelScale, times
            )
    return ticArray
...
def remove_outlier_bboxes(bboxes):
    ...
    for i, b in enumerate(bboxes):
        if not (b is None):
            if (
                (b[0] >= q1_x0 - (1.5 * IQR_x0))
                and (b[0] <= q3_x0 + (1.5 * IQR_x0))
                and (b[1] >= q1_y0 - (1.5 * IQR_y0))
                and (b[1] <= q3_y0 + (1.5 * IQR_y0))
            ):
                out_bboxes[i] = b[:]
            else:
                outliers[i] = b[:]
...
def cut_ROI200(full_array, bboxes, window_loc):
    ...
    for frame in range(full_array.shape[0]):
        ...
        x_center = x0 + int(w / 2)
        y_center = y0 + int(h / 2)
        valid_w = min(int(x_center + cut_w / 2), window_x0 + window_w) - max(
            int(x_center - cut_w / 2), window_x0
        )
        valid_h = min(int(y_center + cut_h / 2), window_y0 + window_h) - max(
            int(y_center - cut_h / 2), window_y0
        )
        ...
        try:
            ROI[
                frame,
                int(center_ROI - valid_h / 2) : int(center_ROI + valid_h / 2),
                int(center_ROI - valid_w / 2) : int(center_ROI + valid_w / 2),
            ] = full_array[
                frame,
                max(int(y_center - cut_h / 2), window_y0) : min(
                    int(y_center + cut_h / 2), window_y0 + window_h
                ),
                max(int(x_center - cut_w / 2), window_x0) : min(
                    int(x_center + cut_w / 2), window_x0 + window_w
                ),
            ]
        except IndexError:
            ROI[
                ...
                frame,
                max(int(y_center - cut_h / 2 + 1), window_y0) : min(
                    int(y_center + cut_h / 2), window_y0 + window_h
                ),
                max(int(x_center - cut_w / 2 + 1), window_x0) : min(
                    int(x_center + cut_w / 2), window_x0 + window_w
                ),
            ]
