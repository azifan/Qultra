...
def paramap(
    ...
    # time/(img.shape[1]+1);
    times = []
    # times = np.arange(1,img.shape[3]+1);
    ...
    for x_base in range(len(xlist)):
        for y_base in range(len(ylist)):
            for z_base in range(len(zlist)):
                ...
                # Do the fitting
                try:
                    params, popt, _, _ = data_fit(
                        cur_TIC, "BolusLognormal", normalizer, time
                    )
                except RuntimeError:
                    print("RunTimeError")
                    ...
                    params = np.array(
                        [
                            np.max(cur_TIC[:, 1]) * normalizer,
                            np.trapz(cur_TIC[:, 1] * normalizer, x=cur_TIC[:, 0]),
                            cur_TIC[np.argmax(cur_TIC[:, 1]), 0],
                            np.max(cur_TIC[:, 0]) * 2,
                            0,
                        ]
                    )
...
def data_fit(TIC, model, normalizer, timeconst):
    ...
    if model == "BolusLognormal":
        ...
        popt, _ = curve_fit(
            ...
            TIC[1],
            p0=(1.0, 3.0, 0.5, 0.1),
            bounds=([0.0, 0.0, 0.0, -1.0], [np.inf, np.inf, np.inf, 10.0]),
            method="trf",
        ...
        sigma = popt[2]
        t0 = popt[3]
        mtt = timeconst * np.exp(mu + sigma * sigma / 2)
        tp = timeconst * exp(mu - sigma * sigma)
        wholecurve = bolus_lognormal(TIC[0], popt[0], popt[1], popt[2], popt[3])
        pe = normalizer * np.max(wholecurve)
        rt0 = timeconst * t0
        # + tp;
        ...
        if tp > 220:
            tp = 220
            # pe = 0.1; rauc = 0.1; rt0 = 0.1; mtt = 0.1;
        if rt0 > 160:
            rt0 = 160
            # pe = 0.1; rauc = 0.1; tp = 0.1; mtt = 0.1;
        if mtt > 2000:
            mtt = 2000
            # pe = 0.1; rauc = 0.1; tp = 0.1; rt0 = 0.1;
        if pe > 1e07:
            pe = 1e07
        if rauc > 1e08:
            rauc = 1e08
        ...
        # ss_res = np.sum(residuals[~np.isnan(residuals)] ** 2)
        # Residual sum of squares
        # ss_tot = np.sum((TIC[1] - np.mean(TIC[1])) ** 2)
        # Total sum of squares
        # r_squared = 1 - (ss_res / ss_tot)
        # R squared
        RMSE = (
            scipy.sum(residuals[~np.isnan(residuals)] ** 2)
            / (residuals[~np.isnan(residuals)].size - 2)
        ) ** 0.5
        # print('RMSE 1');print(RMSE);# RMSE
        # rMSE = (
        #     mean_squared_error(TIC[1], bolus_lognormal(TIC[0], popt[0], mu, sigma, t0))
        #     ** 0.5
        # )
        wholecurve *= normalizer
        # print('RMSE 2');print(rMSE);
...
def read_xmlraw_image_func(filename):
    ...
    numfiles = len(root)
    # Comment this out if using max num frames on next lines
    time = 0
    ...
    for i in range(0, numfiles):
        ...
        if (root[i].find("Geometry") is None) is False:
            ...
            try:
                voxel = [
                    float(physicalDeltaX.text) * 10,
                    float(physicalDeltaY.text) * 10,
                    float(physicalDeltaZ.text) * 10,
                ]  # cm to mm
                P = int(regionLocationMaxz1.text) + 1
...
def read3D(data, newres, cut):  # =[[-1,-1,5,5],[-1,-1,25,5],[-1,-1,5,5]]):
    ...
    imarray = (
        []
    )  # np.zeros((len(xmlnamedir),shapes[2],shapes[1],shapes[0]),dtype='uint8')
    timeinitial = -1000
    ix = -1
    imi_mid, res, timelast, _, _ = read_xmlraw_image_func(
        xmlnamedir[np.uint16(len(xmlnamedir) / 2)]
    )
    imi10, res, timelast, _, _ = read_xmlraw_image_func(xmlnamedir[10])
    imi_mid, res, timelast, _, _ = read_xmlraw_image_func(xmlnamedir[1])
    ...
    for xmlname in xmlnamedir:
        ...
        imi, res, timelast, _, _ = read_xmlraw_image_func(xmlname)
        sz = imi.shape
        ...
        if timeinitial == -1000 and np.sum(cut) != 0:
            ...
            lc = rank.otsu(mp.astype("uint16"), disk(10))
            mpb2 = mp > 0.7 * np.mean(lc)
            mpb = (mpb + mpb2) > 0
            ...
            if N_lines_y_axis_cut[0] != 0:
                N_lines_y_axis_cut[0] = max(
                    0,
                    max(
                        N_lines_y_axis_cut[0] * (ix[0][0] > 0),
                        ix[0][0] - N_lines_y_axis_cut_limit[0],
                    ),
                )
            if N_lines_y_axis_cut[1] != 0:
                N_lines_y_axis_cut[1] = sz[1] - min(
                    sz[1],
                    max(
                        (sz[1] - N_lines_y_axis_cut[1]) * (ix[0][-1] < sz[1]),
                        ix[0][-1] + N_lines_y_axis_cut_limit[1],
                    ),
                )

            if N_lines_x_axis_cut[0] != 0:
                N_lines_x_axis_cut[0] = max(
                    0,
                    max(
                        N_lines_x_axis_cut[0] * (ix0[0][0] > 0),
                        ix0[0][0] - N_lines_x_axis_cut_limit[0],
                    ),
                )
            if N_lines_x_axis_cut[1] != 0:
                N_lines_x_axis_cut[1] = sz[2] - min(
                    sz[2],
                    max(
                        (sz[2] - N_lines_x_axis_cut[1]) * (ix0[0][-1] < sz[2]),
                        ix0[0][-1] + N_lines_x_axis_cut_limit[1],
                    ),
                )

            if N_lines_z_axis_cut[0] != 0:
                N_lines_z_axis_cut[0] = max(
                    0,
                    max(
                        N_lines_z_axis_cut[0] * (iz[0][0] > 0),
                        iz[0][0] - N_lines_z_axis_cut_limit[0],
                    ),
                )
            if N_lines_z_axis_cut[1] != 0:
                N_lines_z_axis_cut[1] = sz[0] - min(
                    sz[0],
                    max(
                        (sz[0] - N_lines_z_axis_cut[1]) * (iz[0][-1] < sz[1]),
                        iz[0][-1] + N_lines_z_axis_cut_limit[1],
                    ),
                )
        ...
        if np.sum(cut) == 0:
            ...
            # reduce matrix size
            imi = imi[
                N_lines_z_axis_cut[0] : sz[0] - N_lines_z_axis_cut[1],
                N_lines_y_axis_cut[0] : sz[1] - N_lines_y_axis_cut[1],
                N_lines_x_axis_cut[0] : sz[2] - N_lines_x_axis_cut[1],
            ]
    ...
    if (
        not len(finalImi)
        or not finalImi.shape[0]
        or not finalImi.shape[1]
        or not finalImi.shape[2]
    ):
        print("Inputted image uses 2d data. Cannot parse into 3d data")
        exit(1)

    time = timelast - timeinitial
    # total time of cine in seconds.
    if newres != 0:
        print("voxel size is changed from ", res, "to voxel size of ", newres)

    print("Reduced image volume size", np.array(finalImi).shape)

    sh1_og = np.shape(imi_mid)
    sh1_og = (sh1_og[0] * sh1_og[1] * sh1_og[2]) / (1024**2)
    sh1 = np.shape(finalImi)
    sh1 = (sh1[0] * sh1[1] * sh1[2]) / (1024**2)
    imarray1 = np.zeros(
        (1, len(imarray), 1, finalImi.shape[0], finalImi.shape[1], finalImi.shape[2]),
        dtype="uint8",
    )
...
def xml2nifti(folderPath, fileDestination):
    ...
    imarray_org, orgres, time = read3D(
        folderPath, 0, [[-1, -1, 5, 5], [-1, -1, 25, 5], [-1, -1, 5, 5]]
    )
    ...
    affine = np.eye(4)
    niiarray = nib.Nifti1Image(imarray_org2, affine)
