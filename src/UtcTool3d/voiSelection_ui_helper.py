...
from pyquantus.parse.philips3dRf import philipsRfParser3d

import src.Utils.utils as ut
from src.UtcTool3d.voiSelection_ui import Ui_constructVoi
from src.UtcTool3d.saveVoi_ui_helper import SaveVoiGUI
from src.UtcTool3d.interpolationLoading_ui_helper import InterpolationLoadingGUI
from src.UtcTool3d.advancedRoi_ui_helper import AdvancedRoiDrawGUI
...
class VoiSelectionGUI(Ui_constructVoi, QWidget):
    def __init__(self):
        ...
        self.fullPhantomPath = None
        self.imgDataStruct, self.imgInfoStruct, self.refDataStruct, self.refInfoStruct = None, None, None, None
        self.data4dImg = None
    ...
    def mousePressEvent(self, event):
        if (self.drawRoiButton.isHidden() or not self.drawRoiButton.isChecked()) and self.painted == "none":
            if self.statusLabel.text() == "Observing!":
                self.statusLabel.setText("Navigating!")
                self.statusLabel.setStyleSheet("QLabel { background-color : rgb(42, 42, 42); color : green; }")
                ...
                self.statusLabel.setText("Observing!")
                self.statusLabel.setStyleSheet("QLabel { background-color : rgb(42, 42, 42); color : red; }")
    ...
    def openImage(self):
        ...
        if not hasattr(self.imgInfoStruct, "frameRate") or not self.imgInfoStruct.frameRate:
            print("Using 1 fps as the frame rate by default...")
            self.imgInfoStruct.frameRate = 1

        self.sliceArray = np.round(
            [i * self.imgInfoStruct.frameRate for i in range(1, self.data4dImg.shape[3] + 1)],
            decimals=2,
        )
    ...
    def axCoordChanged(self, pos):
        if not self.scrollPaused and ((self.statusLabel.text() != "Observing!" and self.painted == "none") or self.painted == "ax"):
            ...
            if xCoord < 0 or yCoord < 0 or xCoord >= self.axialPlane.pixmap().width() or yCoord >= self.axialPlane.pixmap().height():
                return
            self.newYVal = int((xCoord/self.axialPlane.pixmap().width()) * self.y)
            self.newXVal = int((yCoord/self.axialPlane.pixmap().height()) * self.x)
            self.updateCrosshairs()
    ...
    def sagCoordChanged(self, pos):
        if not self.scrollPaused and ((self.statusLabel.text() != "Observing!" and self.painted == "none") or self.painted == "sag"):
            ...
            self.newXVal = int((yCoord/self.sagPlane.pixmap().height()) * self.x)
            self.updateCrosshairs()
    ...
    def corCoordChanged(self, pos):
        if not self.scrollPaused and ((self.statusLabel.text() != "Observing!" and self.painted == "none") or self.painted == "cor"):
            ...
            if xCoord < 0 or yCoord < 0 or xCoord >= self.corPlane.pixmap().width() or yCoord >= self.corPlane.pixmap().height():
                return
            self.newZVal = int((xCoord/self.corPlane.pixmap().width()) * self.z)
            self.newYVal = int((yCoord/self.corPlane.pixmap().height()) * self.y)
            self.updateCrosshairs()

    def updateCrosshairs(self):
        self.updateAdvancedRoiEditButtons()
        self.changeAxialSlices(); self.changeSagSlices(); self.changeCorSlices()
        xCoordAx = int((self.newYVal/self.y) * self.axialPlane.pixmap().width())
        yCoordAx = int((self.newXVal/self.x) * self.axialPlane.pixmap().height())
        xCoordSag = int((self.newZVal/self.z) * self.sagPlane.pixmap().width())
        yCoordSag = int((self.newXVal/self.x) * self.sagPlane.pixmap().height())
        xCoordCor = int((self.newZVal/self.z) * self.corPlane.pixmap().width())
        yCoordCor = int((self.newYVal/self.y) * self.corPlane.pixmap().height())
    ...
    def changeSagSlices(self):
        self.sagittalFrameNum.setText(str(self.newYVal + 1))

        data2dSag = self.data4dImg[:, self.newYVal, :, self.curSliceIndex].astype(np.uint8)
        data2dSag = np.require(data2dSag, np.uint8, "C")
        ...
        tempSag = self.maskCoverImg[:, self.newYVal, :, :]  # 2D data for sagittal
    ...
    def changeCorSlices(self):
        self.coronalFrameNum.setText(str(self.newXVal + 1))

        data2dCor = self.data4dImg[self.newXVal, :, :, self.curSliceIndex]
        data2dCor = np.require(data2dCor, np.uint8, "C")
