...
class RfAnalysisGUI(QWidget, Ui_rfAnalysis):
    ...
    def computeWindowSpec(self):
        ...
        PsGraphDisplayGUI.figure.subplots_adjust(left=0.16,right=0.98, bottom=0.2,top=0.98)
    ...
    def updateLegend(self):
        ...
        if curDisp == "MBF":
            ...
            cbar = self.figLeg.colorbar(orientation='vertical', cax=self.cax, mappable=img)
            ...
            self.cax.set_yticklabels([int(minMBF*10)/10,int((((maxMBF-minMBF)/4)+minMBF)*10)/10,int((((maxMBF - minMBF)/2)+minMBF)*10)/10,int(((3*(maxMBF-minMBF)/4)+minMBF)*10)/10,int(maxMBF*10)/10])
        elif curDisp == "SS":
            ...
            self.cax.set_yticks([0,0.25, 0.5, 0.75, 1])
            self.cax.set_yticklabels([int(minSS*100000000)/100,int((((maxSS-minSS)/4)+minSS)*10000000)/100,int((((maxSS - minSS)/2)+minSS)*100000000)/100,int(((3*(maxSS-minSS)/4)+minSS)*100000000)/100,int(maxSS*100000000)/100])
        elif curDisp == "SI":
            ...
            # plt.tick_params('y', labelsize=5, pad=0.7)
            self.cax.set_yticks([0,0.25, 0.5, 0.75, 1])
            self.cax.set_yticklabels([int(minSI*10)/10,int((((maxSI-minSI)/4)+minSI)*10)/10,int((((maxSI - minSI)/2)+minSI)*10)/10,int(((3*(maxSI-minSI)/4)+minSI)*10)/10,int(maxSI*10)/10])
